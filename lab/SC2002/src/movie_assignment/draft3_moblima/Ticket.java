...
public class Ticket {
	...
	public Ticket(Movie movieChosen, Seat seat, Cineplex cineplex, Cinema cinema, LocalDate date, MovieSlot slot) {
		...
		this.date = date;
		this.slot = slot;
		this.transactionID = String.valueOf(cineplex.getID(cineplex)) + cinema.getCinemaID() + dtf.format(now);
	...
	public static Ticket BookTicket(ArrayList<Movie> movies, ArrayList<MovieSlot> slots, int age) {
		...
		for (MovieSlot ms : slots) {
			...
			if (ms.getCineplex().getID(selectedCineplex) == msFound.getCineplex().getID(selectedCineplex)
				...
				while (true) {
					...
					Seat.printSeats(ms.getCinemaSeats());
					...
					if (!ms.getCinemaSeats()[rowNum][col - 1].isOccupied()) {
