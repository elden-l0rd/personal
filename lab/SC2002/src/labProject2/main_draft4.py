...
def main():
    ...
    for i in range(TEST_CASE): # number of test cases
        for prob in np.arange(0.35,0.95,0.1): #value of p
            if (prob==(0 or 1)): # probability must be nonzero and <1
                prob-=0.1
                continue
            vertices = random.randint(20,1000) # number of vertices
            ver.append(vertices)
    ...
    # M = np.vstack((x,y,z))
    # np.savetxt(r'C:\Users\laley\Downloads\data.csv', M,
    #                                     delimiter=',',
    #                                     header="|V|, density, cputime",
    #                                     fmt='%f')
    ## to CSV method: https://blog.quantinsti.com/creating-heatmap-using-python-seaborn/ ##


    #a= np.array([x],[y],[z])
    #np.savetxt(r'C:\Users\laley\Downloads\data.csv', a, delimiter=' ')
    ...
    # #M = df.pivot_table('density', '|V|', 'cputime')
    # M = df.pivot_table('cputime', 'density', '|V|')
    # divnorm = colors.TwoSlopeNorm(vcenter=0.)
    # ax = sns.heatmap(M, cmap="seismic", norm=divnorm, annot=False, annot_kws={"fontsize":12}, cbar=True, linewidths=0, square=True)
