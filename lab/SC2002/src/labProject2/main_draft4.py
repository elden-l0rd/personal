...
def main():
    ...
    for i in range(TEST_CASE): # number of test cases
        for prob in np.arange(0.35,0.95,0.1): #value of p
            if (prob==(0 or 1)): # probability must be nonzero and <1
                prob-=0.1
                continue
            vertices = random.randint(20,200) # number of vertices
            ver.append(vertices)
    ...
    sns.heatmap(M, cmap="seismic", norm=divnorm, annot=False, annot_kws={"fontsize":12}, cbar=True, linewidths=0, square=True)
